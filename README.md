...
- `landing/CategoryCarousel.tsx` - Auto-advancing category carousel built on Embla. Autoplay pauses while a slide is hovered or focused and the hovered slide lifts and scales outward; under `prefers-reduced-motion` autoplay is not registered at all and the track stays a plain keyboard-navigable list.
- `landing/Reveal.tsx` - Scroll-reveal wrapper. Transitions between two class states rather than firing a one-shot animation, and arms the element by touching the DOM node so scrolling never re-renders.
...
- `scripts/smoke.mjs` - Browser walkthrough driven by Playwright against the running dev server (`npm run dev`, then `npm run smoke`). Signs in with the seeded accounts, walks discovery, the cart drawer, the storefront dialog, the four checkout steps, order tracking, and the merchant dashboard, and asserts the focus traps and Escape handling. Screenshots land in `.smoke/`; the run fails on any console error, page exception, or failed request. Uses the Chrome installed on the machine, so there is no Playwright browser download.
- `scripts/commerce-tests.mjs` - Node test suite for search, cart, and pricing logic. Note it reimplements that logic inline rather than importing from `src/`.
...
- `package.json` - Scripts and dependencies for Next.js, React, Tailwind, Radix (via `radix-ui`), and the Shadcn support libraries (`class-variance-authority`, `clsx`, `tailwind-merge`, `tw-animate-css`, `lucide-react`), and Embla for the landing carousel.
